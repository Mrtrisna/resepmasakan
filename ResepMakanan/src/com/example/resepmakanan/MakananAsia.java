...
public class MakananAsia extends Activity implements TextWatcher, OnItemClickListener{
	private EditText					search;
	private ListView					lv;
	private DatabaseHelperAsia			dbHelper;
	private ArrayAdapter<TableResep>	adapter;
	private List<TableResep>			listResep;

	@Override
	protected void onCreate(Bundle savedInstanceState){
		System.out.println("makanan asia");
		super.onCreate(savedInstanceState);
		setContentView(R.layout.activity_makanan);
		lv = (ListView) findViewById(R.id.listView1);
		lv.setEmptyView(findViewById(R.id.empty));
		search = (EditText) findViewById(R.id.editText1);
		dbHelper = DatabaseHelperAsia.getInstance(this);
		setData();
